...
The game itself needs no Node installation or dependencies. The optional checks require Node, Playwright, and Microsoft Edge (or `--browser` with a browser path). `qa-game.cjs` can also use the bundled Playwright runtime when available.
