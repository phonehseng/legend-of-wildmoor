...
the game needs nothing installed. the optional checks need node, playwright, and edge (or `--browser` plus a browser path). `qa-game.cjs` can use the bundled codex playwright runtime too.
